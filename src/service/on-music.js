...
async function execute(message, serverQueue) {
  ...
  if (!voiceChannel) {
    ...
    return message.channel.send(getEmbed("Tá Errado", "Você tem que estar em um canal de voz!"));
  ...
  if (!serverQueue) {
    ...
    return message.channel.send(getEmbed(song.title + "\n🐺", "Foi Colocado na Fila:" + "\n" + song.url));
...
function skip(message, serverQueue) {
  ...
  if (!message.member.voice.channel) {
    ...
    return message.channel.send(getEmbed("Estou em CDR", "Tem que estar em um canal de voz para pular!"));

  }
  if (!serverQueue) {

    return message.channel.send(getEmbed("Vai pular vento ?", "Não há nada para a pular!"));
...
function clear(message, serverQueue) {
  ...
  if (!serverQueue) {
    ...
    return message.channel.send(getEmbed("TÁ TUDO LIMPO JÁ", "Não há nada a limpar!"));

  }
  serverQueue.songs = [];
  serverQueue.connection.dispatcher.end();

  return message.channel.send(getEmbed("Limpandu", "Pano passado niguém vai ouvir música aqui"))
...
function play(guild, song) {//Function for play music Função que toca a musica
  ...
  serverQueue.textChannel.send(getEmbed(song.title + "\n🐺", "Vai tocar agora\n" + "\n" + song.url));
}